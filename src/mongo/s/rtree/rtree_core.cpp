...
namespace rtree_index
{
	...
	bool rtree_index::RTree::Insert2(OperationContext *txn, mongo::OID NodeOID, mongo::OID &newNode, Branch branch2insert, int Level)
	{
		...
		if (thisLevel > Level)
		{
			...
			if (!Insert2(txn, targetOID, newNode, branch2insert, Level))
			{
				...
				if (old_MBR.MinX != m.MinX)
					hasChanged = true;
				if (old_MBR.MinY != m.MinY)
					hasChanged = true;
				if (old_MBR.MaxX != m.MaxX)
					hasChanged = true;
				if (old_MBR.MaxY != m.MaxY)
					hasChanged = true;
				if (hasChanged)
				{
	...
	bool rtree_index::RTree::Search2(mongo::OID nodeKey, geos::geom::Geometry *SearchGeometry, int Level, vector<mongo::OID> &OIDList, vector<bool> &lazyIntersect)
	{
		...
		if (thisLevel > Level)
		{
			for (int i = 0; i < MaxSize; i++)
			{
				...
				geos::geom::CoordinateSequence *cs = csf.create(5, 2).get();
				...
				geos::geom::LinearRing *pMBR = factory.get()->createLinearRing(cs);
				geos::geom::Polygon *pPolygon = factory.get()->createPolygon(pMBR, NULL);
	...
	bool rtree_index::RTree::splitNode(OperationContext *txn, mongo::OID currentOID, Branch Branch2Insert, mongo::OID &newNode)
	{
		...
		vector<int> ClassA;
		vector<int> ClassB;
		ClassA.push_back(SeedA);
		ClassB.push_back(SeedB);
