...
namespace mongo
{
                                                  ...
                                                  BSONObjBuilder &result) const
    {
        ...
        auto insertStatus = response.toStatus();
        std::cout << "\n\nGot status from response: " << insertStatus.toString() << "\n";
        ...
        if (insertStatus.isOK() && _batchedRequest.getBatchType() == BatchedCommandRequest::BatchType_Insert)
        {
            ...
            std::cout << "We made it into the if statement\n";
            BSONObjBuilder customResult;

            write_ops::InsertCommandRequest request = batchedRequest.getInsertRequest();

            std::vector<BSONObj> docs = request.getDocuments();
            auto collectionName = request.toBSON({})["insert"].valueStringData().toString();
            for (auto const &doc : docs)
            {
                std::cout << doc.toString() << "\n";
                IM.InsertIndexedDoc(opCtx, request.getDbName().db(), collectionName, doc, customResult);
            }
        }
