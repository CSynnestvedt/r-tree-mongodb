...
class CmdInsertIndexedDoc :public BasicCommand
{
public:
    CmdInsertIndexedDoc() :
        BasicCommand("insertIndexedDoc") {
    }

    std::string help() const override {
        return "inserts an indexed document";
    }
    ...
    bool run(OperationContext* opCtx,
        ...
        try {
            ...
            {
                stat = IM.InsertIndexedDoc(opCtx, dbName.db(), collname, cmdObj["documents"].Array()[i].Obj(), result);
            }
        }
        catch (DBException& e) {
            int code = e.code();
        ...
        if (-1 == stat)
            std::string errMsg = "something wrong with rtree index please rebuild it.";
